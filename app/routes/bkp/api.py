...
def create_iptv_source():
    ...
    nome = data.get('nome', '')
    url_m3u = data.get('url_m3u', '')
    url_epg = data.get('url_epg', '')
...
def delete_iptv_source(iptv_id):
    """Delete IPTV source (soft delete)."""
...
def start_classify():
    ...
    try:
        from app.database import get_db
        from app.database.queries import get_all_iptvs
        from app.services.parser import parse_m3u, normalize_name, calculate_hash_midia
        ...
        for iptv in iptv_sources:
            ...
            for i, entry in enumerate(entries):
                ...
                # Check if already exists (incremental processing)
                try:
                    existing = db.execute('''
                        SELECT id FROM midias WHERE hash_midia = ?
                    ''', (hash_midia,)).fetchone()
                except Exception as e:
                ...
                insert_batch.append((
                    ...
                    0, categoria_manual, qualidade, tem_legendado
                ))
        ...
        logger.info(f"Classificação concluída: {total_processed} novas mídias, {total_skipped} ignoradas")
        
        return jsonify({'success': True, 'message': f'Classificação concluída: {total_processed} novas mídias, {total_skipped} ignoradas'})
    except Exception as e:
...
def get_progress():
    ...
    # Use fixed absolute path for database
...
def toggle_blacklist(media_id):
    ...
    db = get_db()
    data = request.get_json()
    blacklist = data.get('blacklist', 1)  # Default to blacklist (1)

    try:
        db.execute('UPDATE midias SET black_list = ? WHERE id = ?', (blacklist, media_id))
        db.commit()
        return jsonify({'success': True})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})
...
def blacklist_batch():
    """Blacklist multiple media items at once."""
    from app.database import get_db
    ...
    try:
        ...
        for media_id in ids:
            ...
            if current:
                ...
                # Update category
                db.execute('UPDATE midias SET categoria = ? WHERE id = ?', (new_category, media_id))
...
def start_tmdb_enrichment():
    ...
    try:
        ...
        for i, item in enumerate(media_items):
            ...
            try:
                tmdb_info = None
                if categoria == 'Movie' or categoria == 'Adult':
                    tmdb_info = get_movie_info(nome)
                elif categoria == 'Series':
                    tmdb_info = get_tv_info(nome)
...
def clean_gallery():
    """Clean all .strm files and empty directories from gallery."""
    try:
        from app.services.config import get_galeria_path
        from pathlib import Path
        import shutil
        ...
        logger.info(f"Caminho da galeria: {galeria_path}")
        logger.info(f"Galeria existe: {galeria_path.exists()}")

        if not galeria_path.exists():
            return jsonify({
                'success': False,
                'error': f'Galeria não encontrada: {galeria_path}'
            })

        # Remove all .strm files from filesystem
        removed_files = 0
        for strm_file in galeria_path.rglob('*.strm'):
            try:
                strm_file.unlink()
                removed_files += 1
                if removed_files % 1000 == 0:
                    logger.info(f"Removidos {removed_files} arquivos...")
            except Exception as e:
                logger.error(f"Erro ao remover {strm_file}: {e}")

        # Remove empty directories recursively (excluding category folders)
        removed_dirs = 0
        category_folders = ['FILMES', 'SERIES', 'NOVELAS', 'DESENHOS', 'DOCUMENTARIOS', 'EDUCACIONAL', 'ESPORTES', 'FILMES_XXX', 'TV']
        ...
        def remove_empty_dirs(path):
            ...
            try:
                ...
                path.rmdir()
                removed_dirs += 1
                if removed_dirs % 1000 == 0:
                    logger.info(f"Removidos {removed_dirs} diretórios...")
                return True
            except OSError:
        ...
        for dir_path in sorted(galeria_path.rglob('*'), key=lambda x: len(x.parts), reverse=True):
            if dir_path.is_dir():
                # Skip category folders at root level
                if dir_path.name in category_folders and dir_path.parent == galeria_path:
                    continue
        ...
        logger.info(f"Galeria limpa: {removed_files} arquivos .strm e {removed_dirs} diretórios vazios removidos")
        return jsonify({
            'success': True,
            'message': f'Galeria limpa: {removed_files} arquivos .strm e {removed_dirs} diretórios vazios removidos'
        })
    except Exception as e:
